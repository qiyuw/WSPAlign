...
from squad_utils import squad_convert_example_to_features as new_squad_convert_example_to_features
from squad_utils import MySquadExample
from squad_utils import divide_by_whitespace_langs
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        processor = MySquadProcessor(tokenizer=tokenizer)
...
def squad_convert_example_to_features_try(example, max_seq_length, doc_stride, max_query_length, is_training):
    try:
        ret = new_squad_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, "max_length", is_training)
...
def squad_convert_examples_to_features(examples, tokenizer, max_seq_length, doc_stride, max_query_length, is_training, feature_writer, threads=1):
    ...
    threads = 1 # single thread is much faster.
    if threads <= 1:
        unique_id = 1000000000
        example_index = 0
        tmp_features = []
        for example in tqdm(examples, total=len(examples), desc="convert squad examples to features"):
            example_features = new_squad_convert_example_to_features(
                example=example,
        ...
        with Pool(threads, initializer=squad_convert_example_to_features_init, initargs=(tokenizer,)) as p:
            annotate_ = partial(
                new_squad_convert_example_to_features,
                max_seq_length=max_seq_length,
                doc_stride=doc_stride,
                max_query_length=max_query_length,
                padding_strategy='max_length',
...
class MySquadProcessor(SquadV2Processor):
    ...
    def _create_examples(self, input_data, set_type):
        ...
        for entry in tqdm(input_data):
            title = entry["title"] if "title" in entry.keys() else "no_title"
            for paragraph in tqdm(entry["paragraphs"]):
                context_text = paragraph["context"]
                if "lang" in paragraph.keys():
                    lang = paragraph["lang"]
                else:
                    lang = "UNK"
                if "doc_tokens" in paragraph.keys(): # pre-divided tokens are prior.
                    doc_tokens = paragraph["doc_tokens"]
                    doc_tokens, char_to_word_offset = MySquadExample.predifined_divide_context_text(context_text, doc_tokens)
                elif lang == 'ja':
                    doc_tokens, char_to_word_offset = MySquadExample.ja_divide_context_text(context_text, self.tokenizer)
                elif lang == 'zh':
                    doc_tokens, char_to_word_offset = MySquadExample.zh_divide_context_text(context_text)
                elif lang in divide_by_whitespace_langs or lang == 'UNK':
        ...
        print('{} examples skipped.'.format(num_skipped_examples))
        return examples