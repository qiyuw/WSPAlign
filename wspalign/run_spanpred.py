...
"""
original: huggingface/transformers v4.0.1 examples/question-answering/run_squad.py
"""
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        for key, value in results.items():
                            result_info_li.append("eval_{}: {}".format(key, value))
                            tb_writer.add_scalar("eval_{}".format(key), value, global_step)
                        result_info = " | ".join(result_info_li)
                        logger.info("Global_step: {} | {}".format(global_step, result_info))
                if args.local_rank in [-1, 0] and global_step % 100 == 0:
                    # always write lr and loss every 100 steps
                    tb_writer.add_scalar("lr", scheduler.get_lr()[0], global_step)
                    tb_writer.add_scalar("loss", (tr_loss - logging_loss) / 100, global_step)
                    logging_loss = tr_loss

                # Save model checkpoint
                if global_step == t_total or (args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0):
